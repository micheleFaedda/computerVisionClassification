...
function [acc_SVM_CHI2_train,acc_SVM_CHI2_val,acc_SVM_CHI2_test] = test_multidense_SIFT_pyramid16(path,k_kmeans)
clc
...
for i = 1:length(data)
    ...
    for j = 1:length(images_descs)
        for l = 1 : 16
            fname = fullfile(basepath,'img',dataset_dir,data(i).classname,[images_descs{j}(1:end-18),'_',num2str(l+4),'.all_pyramid_color']);
            tmp = load(fname,'-mat');
            tmp.desc.class=i;
