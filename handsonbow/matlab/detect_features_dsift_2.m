...
function detect_features_dsift_2(im_dir,file_ext,varargin)
...
parfor i = 1:length(dd)
    ...
    for psize=1:length(scales);
        [sift_tmp,gx{psize},gy{psize}]=sp_dense_sift(im2gray(I),stride,scales(psize));
    ...
    fname_out = [fname(1:end-4),'_',num2str(24),'.','all_pyramid_color'];
    fname_out
